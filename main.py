...
def leer_datos(excel_path, fecha_inicio_str, fecha_fin_str):
    ...
    for row in ws_asignaciones.iter_rows(min_row=2, values_only=True):
        fecha_str = row[0]
        if isinstance(fecha_str, datetime):  # Verificamos si es un objeto datetime
            fecha = fecha_str.date()  # Extraemos solo la fecha sin la hora
        else:
            fecha = datetime.strptime(fecha_str, '%Y-%m-%d').date()  # Convertimos manualmente si es string
